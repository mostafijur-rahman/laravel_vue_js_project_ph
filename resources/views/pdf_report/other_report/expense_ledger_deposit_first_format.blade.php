				...
				<table class="table-design" cellpadding="0" cellspacing="0">
                    ...
                    @if(count($general_expenses) > 0)
                        @php 
							...
							$sum_amount = 0;
						@endphp
                        @foreach($general_expenses as $key => $list)
                            @php 
                                $right_balance += $list->amount; 
                                $sum_amount += $list->amount; 
                            @endphp
                            <tr class="item">
                                <td class="center">{{ \Carbon\Carbon::parse($list->date_time)->format('d M, Y') }}</td>
                                <td class="center">{{ $list->expense_info->name }}</td>
                                <td class="center">{{ number_format($list->amount) }}</td>
                                <td class="center">{{ number_format($right_balance) }}</td>
                            </tr>
                        @endforeach
						<tr>
							<td class="text-right" colspan="2"><b style="font-weight: bold;">@lang('cmn.total') = </b></td>
