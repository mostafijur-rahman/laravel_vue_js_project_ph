			...
			<div class="invoice-box">
				<table class="table-design" cellpadding="0" cellspacing="0">
                    ...
                    <tr>
                        ...
                        <td class="text-left">
                            @lang('cmn.vehicle'): <b>{{ $list['number_plate'] ? $list['number_plate'] :'---' }}</b><br>
                            @lang('cmn.driver'): <b>{{ $list['driver_name']? $list['driver_name'] :'---' }} {{ $list['driver_phone'] ? '(' . $list['driver_phone'] . ')' :'' }}</b><br>
                            @lang('cmn.helper'): <b>{{ $list['helper_name']? $list['helper_name'] :'---' }} {{ $list['helper_phone'] ? '(' . $list['helper_phone'] . ')' :'' }}</b><br>
                            
                            @lang('cmn.load_point'): <b>{{ $list->load? $list->load :'---' }}</b><br>
                            @lang('cmn.unload_point'): <b>{{ $list->unload? $list->unload :'---' }}</b>
                        </td>
						...
						<td class="text-right">
							@lang('cmn.total_deposit') = {{ number_format(array_sum(array_column($list['client_payments']->ToArray(), 'amount'))) }}<br>
							@lang('cmn.total_expense') = (-) {{ number_format(
								array_sum(array_column($list['general_expenses']->ToArray(), 'amount')) +
								array_sum(array_column($list['oil_expenses']->ToArray(), 'amount'))
								) }}<br>
							---------------------------------------------- <br>
							@lang('cmn.balance') = {{ number_format(
								array_sum(array_column($list['client_payments']->ToArray(), 'amount')) - 
								(
									array_sum(array_column($list['general_expenses']->ToArray(), 'amount')) +
									array_sum(array_column($list['oil_expenses']->ToArray(), 'amount'))
						...
						<td class="text-right">
							@if(count($list['general_expenses']) > 0 || count($list['oil_expenses']) > 0)

								@if (count($list['oil_expenses']) > 0)
									@foreach ($list['oil_expenses'] as $oil_expense)
										<p>
											@lang('cmn.fuel') (@lang('cmn.liter') =  {{ number_format($oil_expense->liter) }} , @lang('cmn.rate') =  {{ number_format($oil_expense->rate) }}) = <b>{{ number_format($oil_expense->amount) }}</b>
										</p>
									@endforeach
								@endif

								@if (count($list['general_expenses']) > 0)
									@foreach ($list['general_expenses'] as $general_expense)
										<p>
											{{ $general_expense->expense_info->name }} =  <b>{{ number_format($general_expense->amount) }}</b>
										</p>
									@endforeach
								@endif
								---------------------------------------------- <br>
								@lang('cmn.total') = {{ number_format(
									array_sum(array_column($list['general_expenses']->ToArray(), 'amount')) +
									array_sum(array_column($list['oil_expenses']->ToArray(), 'amount'))
								) }}
							@else
