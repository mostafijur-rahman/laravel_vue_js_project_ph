<template>
    <div>
        <div class="modal fade" id="showVendorModal">
...
export default {
	...
	methods: {
        addUpdate(){
			...
			// update
			if(this.form.id){
				request = axios.put('/api/vendors/' + this.form.id, this.form)
			// insert
			} else {
				request = axios.post('/api/vendors', this.form)
			}
