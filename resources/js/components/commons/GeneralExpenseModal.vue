<template>
    <div>
        <div class="modal fade" id="showGeneralExpenseModal">
            <div class="modal-dialog modal-xl">
                <div class="modal-content">
                    
                    <div class="modal-header">
                        <h4 class="modal-title">{{ $t('general_expense') }}</h4>
                                ...
                                <table class="table table-striped text-center table-hover">
                                    ...
                                    <tbody>
                                        ...
                                        <tr v-if="(generalExpenses.length > 0)" v-for="(expense, expenseIndex) in generalExpenses">
                                            ...
                                            <td>{{ expense.expense_info?expense.expense_info.name:'---' }}</td>
                                        ...
                                        <tr>
                                            ...
                                            <td class="text-center">
                                                <b v-if="(generalExpenses.length > 0)">{{ $numFormat($getSumFromLists(generalExpenses, 'amount')) }}</b>
                                            </td>
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-4">
                                    <div class="form-group">
                                        <label for="">
                                            {{ $t('vendor') }}
                                            <small v-if="$store.state.auth.user.company.loan_feature == 'enable' && form.account_id == 'loan'" class="text-danger">({{ $t('required') }})</small>
                                        </label>
                                        <VueMultiselect
                                ...
                                <div class="col-md-4">
                                    <div class="form-group">
                                        <label for="">{{ $t('expense_head') }} <small class="text-danger">({{ $t('required') }})</small></label>
                                        <VueMultiselect
                                            :class="{ 'is-invalid': (validationErrors && validationErrors['expense_id'])?true:false }"
                                            v-model="form.expense_id"
                                            :options="settingExpenses"
                                            :multiple="false"
                                        ...
                                        <span
                                            class="error invalid-feedback"
                                            v-if="(validationErrors && validationErrors['expense_id'])?true:false"
                                            v-for="(error) in validationErrors['expense_id']"
                                            >{{ error }}
                                        </span>
                                    </div>
                                </div>
...
export default {
    ...
    props: {
        ...
        vendorType: { type: [String], default: 'other'},
        settingExpenses: { type: [Array], default: {}},
    },
    components: {
        VueMultiselect
    },
	...
	methods: {
		...
		async destroy(id, index) {
			try {
                ...
                this.generalExpenses.splice(index, 1);
                this.$emit('refreshGeneralExpense', this.filter.challan_id);
        ...
        addExpense(name){
            ...
            let newForm = { name : name, status : 'active'}

            axios
                .post('/api/settings/expenses', newForm)
                .then((response) => {
                    ...
                    this.$emit("addExpense", value);
                    this.form.expense_id = '';
                    this.form.expense_id = value;
