								...
								<div class="row">
									<div class="col-md-3">
										<div class="form-group">
											<input type="text" class="form-control" v-model="filter.name" :placeholder="$t('name_here')">
										</div>
									</div>
									<div class="col-md-5">
										<div class="form-group">
											<button 
												...
												@click="getData()"
												>
												<i class="fa fa-filter"></i> {{ $t('search') }}
											</button>
											<button 
												type="button" 
...
export default {
	...
	methods: {
		...
		getData() {
			...
			axios.get('/api/settings/banks', { params })
				.then((response) => {
		...
		createData() {
			...
			this.loading = true;
			// insertion
			axios.post('/api/settings/banks', this.form)
				.then((response) => {
		...
		updateData() {
			...
			this.loading = true;
			// insertion
			axios.put('/api/settings/banks/' + this.form.id, this.form)
				.then((response) => {
		...
		async destroy(id, index) {
			try {
				...
				// fire request
				let response = await axios.delete('/api/settings/banks/' + id);
	...
	computed: {
        submitBtnIcon: function() {
            return (this.submitLoading)?'fas fa-circle-notch fa-spin' : 'fas fa-save';
        }
    },
}
</script>