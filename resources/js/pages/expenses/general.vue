										...
										<tr v-if="(lists.length > 0)" v-for="(list, index) in lists">
											...
											<td class="text-left">
												<!-- <small v-if="list.transaction && list.transaction.account">
													{{ $t('account_number') }} : <b>{{ (list.transaction && list.transaction.account.account_number)? list.transaction.account.account_number : $t('cash_account') }}</b><br> 
													{{ $t('holder_name') }} : <b>{{ (list.transaction  && list.transaction.account.holder_name )? list.transaction.account.holder_name :'---' }}</b><br> 
	...
	methods: {
		...
		showGeneralExpenseModal(indexId = null, list = null){
			...
			if(list){
				...
				this.$refs.gERef.form.account_id = list.transaction?list.transaction.account_id: list.loan?'loan':'';
