...
use App\Http\Requests\Expense\GeneralRequest;
...
class GeneralController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $query = General::query()->with('transaction.account.bank_info', 'loan.transactions', 'expense_info', 'vendor_info', 'vehicle_info', 'challan_info', 'created_info', 'updated_info');
        ...
        if($request->input('withoutChallan')){
            $query = $query->whereNull('challan_id');
        }


        if($request->input('paid_status')){

            if($request->input('paid_status') == 'due'){
                $query = $query->whereHas('loan');
            } elseif($request->input('paid_status') == 'paid'){
                $query = $query->doesnthave('loan');
            }
    ...
    {
        // loan enable thakle then, trans == loan hole then , vendor required thakbe
        // payment enable thakle then, account id reqired thakbne
        ...
        try {
            ...
            if($request->input('expense_id') && $request->input('expense_id')['id']){
                $model->expense_id = $request->input('expense_id')['id'];
            }
            ...
            if($this->loan_feature() == 'enable' && $request->input('account_id') == 'loan'){
                ...
                $loans['purposeable_type'] = 'expense_generals';
            ...
            if($this->payment_feature() == 'enable' && $request->input('account_id') != 'loan'){
                ...
                $trans['transactionable_type'] = 'expense_generals';
                $trans['type'] = 'out';
    ...
    public function update(Request $request, $id)
    {
        ...
        try{
            ...
            $model = General::where('id', $id)->with('transaction')->first();
            $model->vendor_id = $request->input('vendor_id');
            $model->vehicle_id = $request->input('vehicle_id');
            $model->challan_id = $request->input('challan_id');
            $model->expense_id = $request->input('expense_id');
            $model->voucher_number = $request->input('voucher_number');
            $model->amount = $request->input('amount');
